...
export function collectRiskData(clientMetadataID, env) {
  ...
  */
  ...
  var _setCookiesDataOnPayload = function _setCookiesDataOnPayload() {
    ...
    payload.cookies[vidCookieName] = null; // checking if local storage exists

    if (window.localStorage.getItem(constants.SC_COOKIE)) {
      var _payload$cookies$push;

      payload.cookies.push((_payload$cookies$push = {}, _payload$cookies$push[constants.SC_COOKIE] = window.localStorage.getItem(constants.SC_COOKIE), _payload$cookies$push));
    }

    if (window.localStorage.getItem(vidCookieName)) {
      var _payload$cookies$push2;

      payload.cookies.push((_payload$cookies$push2 = {}, _payload$cookies$push2[vidCookieName] = window.localStorage.getItem(vidCookieName), _payload$cookies$push2));
    }
  };
  /**
